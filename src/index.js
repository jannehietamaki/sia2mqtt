...
const topic = "sia2mqtt";

const client = mqtt.connect(process.env.MQTT || "mqtt://localhost:1883");
 ...
 */
const crc16str = (str) => crc16(Buffer.from(str)).toString(16).toUpperCase().padStart(4, "0");

/*
 *  Calculate the size of a message and transform to a 4 zero-padding string in hex
 */
const msgSize = (str) => str.length.toString(16).toUpperCase().padStart(4, "0");
 ...
 */
const server = (dispatch  ) => net.createServer((socket) => {
  socket.on('error', (err) => {
    console.error(err)
  });
  socket.on('data', (data) => {
    ...
    const status = socket.write(response);
  });
});
  ...
  client.subscribe(topic, (err) => {
    if (!err) {
      server((data) => {
        ...
        console.log(data);
      }).listen(Number(process.env.PORT || 6000));
    }
  })
})
